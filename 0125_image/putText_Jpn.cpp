...
//
static void sttc_putTextCore(cv::Mat& a_r_img_dst, std::wstring a_p_text, cv::Point a_pos_org, const TCHAR* a_p_fontname, double a_font_scale, cv::Scalar a_font_color, int a_thickness, int a_lineType)
{
	// 未使用
	...
	::TextOut(hdc_Compatible, 0, y_org, a_p_text.c_str(), str_size);
...
//
void sc::myCV::putText_Jpn(cv::Mat& a_r_img_dst, std::wstring a_p_text, cv::Point a_pos_org, const TCHAR* a_p_fontname, double a_font_scale, cv::Scalar a_font_color, int a_thickness, int a_lineType)
{
	// a_r_img_dstが空ではない
